	...
	<body data-dir-all="<?php echo $dirDirDir; ?>">
						...
						<tr class='new_folder'>
							<td colspan="2" align="right">
								<form class='newFolder'>
									...
									<input type='text' id="newFolder" name='new_folder' class='inputFolderName' placeholder='enter folder name here...'>
									<button class='buttonFolderNew'>go</button><br />
						...
						<tr class='detailClicked'>
							<td colspan="2" align="left">								
								...
								<ul class="menuSelected parentMenu">
									<li class="dwnldbtn">
										<a class='btnSub' data-show='subEdit' data-hide="parentMenu">Edit</a>
									</li>
									<li>
										<a class='btnSub' data-show='subRename' data-hide="parentMenu">Rename</a>
									</li>
									<li class="unzipBtn">
										<a class='btnSub' data-show='subUnzip' data-hide="parentMenu">Extract</a>
									</li>
									<li class="dwnldbtn">
										<a class='downloadBtn' href='' download>Download</a>
									</li>
									<li>
										<a class='btnSub' data-show='subDelete' data-hide="parentMenu">Delete</a>
									</li>
								</ul>
								<ul class="menuSelected subDelete subMenu">
									<li>Are you sure? 
										<a class="yesDeleteBtn" data-path="">Yes</a> 
										or 
										<a class="btnSub" data-hide="subMenu" data-show="parentMenu">Cancel</a>
									</li>
								</ul>
								
								<form class="extractTo">
									<input type='hidden' class="getZipPath" name='getZipPath' value=''>
									<ul class="menuSelected subUnzip subMenu">
										<li>
											Extract to : 
											<input type='text' placeholder='enter directory name' class='inputRename newDirName' value="" name='dirNameUnzip'>
											<input type='submit' value='go' class="goRename">
										</li>
									</ul>
								</form>

								<form class="subEdit">
									<div class="menuSelected subEdit subMenu">
										Filename : <input type="text" name="fName" class="fName">
										<div>
											<textarea name="" class="dFata"></textarea>
										</div>
									</div>
								</form>
								...
								<form class='renameSelected'>
									...
									<ul class="menuSelected subRename subMenu">
										<li>
											Rename to : 
											<input type='text' placeholder='enter new name' class='inputRename newNameN' value="" name='newName'>
											<input type='submit' value='go' class="goRename">
										</li>
									</ul>
								</form>
								...
								foreach ($data_list->data as $key => $value) {
									...
									if(isset($value->name)){
										...
										if($value->directory){
											$kind 	= "folder";
											$size 	= "";
											$name 	= "<a href='#file=' dir='".str_replace("+", "%2B", $myrootdir).str_replace("+", "%2B", $name)."' class='goDir'>".$name."</a>";
											$class_add	.= "thisDir";
										}
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				$(document).on("submit",".editSelected",function(e){
					e.preventDefault();
					var data_f 	= $(this).serialize();
					loadingStart();
					$.ajax({
				...
				$(document).on("click",".clickable",function(){
					...
					$(".dFata").val(readTextFile(selectedName));
